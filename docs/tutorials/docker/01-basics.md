...
::: tip 关键概念
- **镜像（Image）** 是只读模板，由一层层只读层（layer）叠加而成；**容器（Container）** 是镜像的运行实例，在镜像之上加了一层可写层。
- 多个容器可共享同一镜像，互不影响；改了一个容器的可写层不会污染镜像本身。
- **分层（layer）** 是 Docker 缓存与增量分发的基础：相同的基础层只需下载/存储一次。
:::
...
容器可写层是临时的——**容器删除后数据即丢失**。要持久化数据，必须挂载外部存储。Docker 提供两种主要方式：

::: tip 关键点
- **Volume（卷）**：由 Docker 管理的命名存储（位于 `/var/lib/docker/volumes/`），推荐用于生产，性能好、易备份。
- **Bind Mount**：把宿主机目录直接挂进容器，适合开发时实时同步代码，**生产慎用**（易误改宿主机文件）。
- 数据库等有状态服务务必使用 Volume，否则 `docker rm` 后数据全失。
:::
