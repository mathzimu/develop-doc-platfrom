...
HTML（HyperText Markup Language）是构建网页的标准标记语言。所有网页的基础骨架都由 HTML 构成。

## 什么是 HTML？

HTML 使用一组标签（tags）来描述网页的内容结构。浏览器读取 HTML 并将其渲染为可视化的页面。HTML 不是编程语言，而是一种标记语言——它告诉浏览器如何组织标题、段落、图片、链接等内容。
