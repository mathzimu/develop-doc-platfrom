...
```js
require('./module')    // 相对路径，先找 ./module.js → ./module.json → ./module/index.js
require('lodash')      // 核心模块 > node_modules
...
console.log(stats.size, stats.birthtime, stats.isFile())
```
...
path.relative('/data/abc', '/data/def')  // '../def'
...
console.log(url.hostname, url.port, url.searchParams.get('q'))
```
